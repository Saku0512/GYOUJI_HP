...
const CACHE_NAME = `tournament-app-${CACHE_VERSION}`;
const STATIC_CACHE_NAME = `tournament-static-${CACHE_VERSION}`;
const DYNAMIC_CACHE_NAME = `tournament-dynamic-${CACHE_VERSION}`;
const API_CACHE_NAME = `tournament-api-${CACHE_VERSION}`;
const IMAGE_CACHE_NAME = `tournament-images-${CACHE_VERSION}`;
...
const MAX_IMAGE_CACHE_SIZE = 50; // 最大50個の画像をキャッシュ
...
const CACHE_STRATEGIES = {
  ...
  // 画像: Cache First with optimization
  images: {
    pattern: /\.(ico|png|jpg|jpeg|gif|svg|webp)$/,
    strategy: 'cacheFirst',
    cacheName: IMAGE_CACHE_NAME,
    maxAge: 30 * 24 * 60 * 60 * 1000, // 30日
    maxEntries: MAX_IMAGE_CACHE_SIZE
  },
